...
CREATE TABLE `patient`  (
  ...
  `is_marry` tinyint(0) NULL DEFAULT NULL COMMENT '是否结婚',
  ...
  `is_insure` tinyint(0) NULL DEFAULT 0 COMMENT '是否有医保',
  `status` tinyint(0) NOT NULL DEFAULT 0 COMMENT '状态（0：默认 1：已认证）',
...
CREATE TABLE `user_info`  (
  ...
  `auth_status` tinyint(0) NOT NULL DEFAULT 0 COMMENT '认证状态（0：未认证 1：认证中 2：认证成功 -1：认证失败）',
